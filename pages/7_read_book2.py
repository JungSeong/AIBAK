...
def main():
    ...
    # ----------------- Read Book ----------------- #
    book_image3_path = './contents/image/scene_2_0.png'
    book_image4_path = './contents/image/scene_2_1.png'

    if not os.path.exists(book_image4_path):
        generate_image.generate_image(2)

    with st.expander(" ", expanded=True):
        image3, image4 = st.columns(2)
        # 생성된 동화 삽화
        book_image3 = Image.open(book_image3_path)
        image3.image(book_image3)
        book_image4 = Image.open(book_image4_path)
        image4.image(book_image4)

        # 생성된 동화 내용
        csv_file_path = './contents/text/story.csv'
        book_story = pd.read_csv(csv_file_path)
        story_line = ' '.join(book_story['story'].tolist()[6:12])
        ...
        if not os.path.exists('./contents/image/scene_3_1.png'):
            # 효과음, TTS 재생
            # 다음 페이지 생성하기 (병렬처리)
            thread_generate_image = multiprocessing.Process(target=generate_image.generate_image, args=(3,))
            thread_generate_sound = multiprocessing.Process(target=generate_sound.generate_sound, args=(3,))
            thread_generate_tts = multiprocessing.Process(target=generate_sound.generate_tts, args=(3,))
    
            thread_generate_image.start()
            thread_generate_sound.start()
            thread_generate_tts.start()
    
            display_sound.display_sound(2)
