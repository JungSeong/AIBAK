...
def main():
    ...
    # ----------------- Read Book ----------------- #
    book_image5_path = './contents/image/scene_3_0.png'
    book_image6_path = './contents/image/scene_3_1.png'

    if not os.path.exists(book_image6_path):
        generate_image.generate_image(3)

    with st.expander(" ", expanded=True):
        image5, image6 = st.columns(2)
        # 생성된 동화 삽화
        book_image5 = Image.open(book_image5_path)
        image5.image(book_image5)
        book_image6 = Image.open(book_image6_path)
        image6.image(book_image6)

        # 생성된 동화 내용
        csv_file_path = './contents/text/story.csv'
        book_story = pd.read_csv(csv_file_path)
        story_line = ' '.join(book_story['story'].tolist()[12:18])
        ...
        if not os.path.exists('./contents/image/scene_4_1.png'):
            # 효과음, TTS 재생
            # 다음 페이지 생성하기 (병렬처리)
            thread_generate_image = multiprocessing.Process(target=generate_image.generate_image, args=(4,))
            thread_generate_sound = multiprocessing.Process(target=generate_sound.generate_sound, args=(4,))
            thread_generate_tts = multiprocessing.Process(target=generate_sound.generate_tts, args=(4,))
    
            thread_generate_image.start()
            thread_generate_sound.start()
            thread_generate_tts.start()
    
            display_sound.display_sound(3)
        ...
        if bt2.button("이전 페이지"):
            # 다음 페이지 넣기
            st.switch_page("./pages/7_read_book2.py")
