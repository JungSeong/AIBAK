...
def main():
    ...
    # ----------------- Read Book ----------------- #
    book_image1_path = './contents/image/scene_1_0.png'
    book_image2_path = './contents/image/scene_1_1.png'

    if not os.path.exists(book_image2_path):
        generate_image.generate_image(1)

    with st.expander(" ", expanded=True):
        image1, image2 = st.columns(2)
        # 생성된 동화 삽화
        book_image1 = Image.open(book_image1_path)
        image1.image(book_image1)
        book_image2 = Image.open(book_image2_path)
        image2.image(book_image2)

        # 생성된 동화 내용
        csv_file_path = './contents/text/story.csv'
        book_story = pd.read_csv(csv_file_path)
        story_line = ' '.join(book_story['story'].tolist()[0:6])
        ...
        if not os.path.exists('./contents/image/scene_2_1.png'):
            # 효과음, TTS 재생
            # 다음 페이지 생성하기 (병렬처리)
            thread_generate_image = multiprocessing.Process(target=generate_image.generate_image, args=(2,))
            thread_generate_sound = multiprocessing.Process(target=generate_sound.generate_sound, args=(2,))
            thread_generate_tts = multiprocessing.Process(target=generate_sound.generate_tts, args=(2,))

            thread_generate_image.start()
            thread_generate_sound.start()
            thread_generate_tts.start()

            display_sound.display_sound(1)
